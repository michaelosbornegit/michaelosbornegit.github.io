...
type Button = {
    ...
    // It's optional when onClick is provided.
    link?: string;
    // The label prop is a string that determines the text of the button.
    label: string;
    // The onClick prop is an optional function that will be called when the button is clicked.
...
export default function Button({
    ...
    className,
    style,
    link = '#', // Default link to prevent issues when neither link nor onClick is provided
    label,
    onClick,
}: Button) {
    ...
    // If onClick is defined, render a button with onClick handler
    // Otherwise, render a Link with a button inside
    if (onClick) {
        return (
            <button
                className={clsx(
